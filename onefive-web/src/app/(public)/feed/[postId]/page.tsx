...
import { FeedAuthSwitch } from './FeedAuthSwitch';
import { PostPrivateSection } from '@/app/(public)/post/[id]/PostPrivateSection';
...
export async function generateMetadata({
  ...
  const contentPreview = post.content ? post.content.slice(0, 140).replace(/\n/g, ' ') : 'Publication sur Onefive';
...
export default async function FeedPostPublicPage({
  params,
}: {
  params: Promise<{ postId: string }>;
}) {
  ...
  const avatarUrl = post.author.avatarId ? `${API_URL}/file/${post.author.avatarId}` : null;

  return (
    <FeedAuthSwitch postId={postId}>
      <div className="min-h-screen bg-[#FCFCFD]">
        <div className="mx-auto max-w-2xl px-4 py-8 sm:px-6 lg:px-8">
            ...
            <div className="p-6">
              <div className="flex items-center gap-3">
                <Link href={`/profile/${post.author.id}`}>
                  {avatarUrl ? (
                    <img src={avatarUrl} alt={authorName} className="h-11 w-11 rounded-full object-cover border border-gray-200" />
                  ) : (
                ...
                <div className="flex flex-col">
                  <Link href={`/profile/${post.author.id}`} className="font-semibold text-[#101828] hover:text-violet-600 transition-colors">
                    {authorName || 'Anonyme'}
                  </Link>
                  <div className="flex items-center gap-2 text-xs text-[#475467]">
                    {post.author.highlight && <span>{post.author.highlight}</span>}
          ...
          <PostPrivateSection postId={postId} />
        </div>
      </div>
    </FeedAuthSwitch>
  );
