...
import { DiscussionAuthSwitch } from '@/app/(public)/discussions/[id]/DiscussionAuthSwitch';
...
export async function generateMetadata({
  ...
  if (!discussion) {
    return { title: 'Discussion non trouvée' };
  }

  const pageTitle =
    discussion.question.length > TITLE_TRUNCATE
      ? `${discussion.question.slice(0, TITLE_TRUNCATE)}...`
      : discussion.question;
  const fullTitle = `${pageTitle} | Onefive`;
  const description =
    discussion.content?.slice(0, 160) ||
    discussion.question.slice(0, 160) ||
    'Discussion sur Onefive';
  ...
  return {
    ...
    openGraph: {
      ...
      images: [
        {
          url: `${baseUrl}/api/og/discussion/${id}`,
          width: 1200,
          height: 630,
          alt: pageTitle,
        },
...
function buildJsonLd(discussion: DiscussionSeoData, baseUrl: string) {
  ...
  return {
    ...
    headline: discussion.question,
    text: discussion.content || discussion.question,
    author: {
      '@type': 'Person',
    ...
    interactionStatistic: [
      {
        ...
        userInteractionCount: discussion.stats.answers,
      },
    ],
...
export default async function DiscussionPublicPage({
  params,
}: {
  params: Promise<{ id: string }>;
}) {
  ...
  const baseUrl = process.env.NEXT_PUBLIC_URL_PUBLIC || 'https://onefive.app';
  const jsonLd = buildJsonLd(discussion, baseUrl);

  const truncatedContent =
    discussion.content && discussion.content.length > CONTENT_TRUNCATE
      ? `${discussion.content.slice(0, CONTENT_TRUNCATE)}...`
      : discussion.content || '';

  const truncatedContext =
    discussion.context && discussion.context.length > 80
      ? `${discussion.context.slice(0, 80)}...`
      : discussion.context || '';
      ...
      <div className="mx-auto max-w-7xl px-4 py-8 sm:px-6 lg:px-8">
        ...
        <Card className="overflow-hidden bg-card rounded-xl border shadow-sm">
          <div className="p-6">
            ...
            {truncatedContent && (
              <p className="mt-4 text-sm text-[#475467] whitespace-pre-wrap">
                {truncatedContent}
              </p>
            )}
        ...
        {/* Private section (blurred answers) */}
