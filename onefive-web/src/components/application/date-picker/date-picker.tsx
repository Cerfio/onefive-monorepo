...
export const DatePicker = ({ value: valueProp, defaultValue, onChange, onApply, onCancel, ...props }: DatePickerProps) => {
    ...
    // Normalize value to ensure it's a valid DateValue or null
    const normalizeValue = (val: DateValue | null | undefined): DateValue | null => {
        if (!val) return null;
        // Check if it's a valid DateValue by checking for the toDate method
    ...
    const normalizedValueProp = normalizeValue(valueProp);
    const normalizedDefaultValue = normalizeValue(defaultValue);
    
    const [value, setValue] = useControlledState(normalizedValueProp, normalizedDefaultValue, onChange);

    const formattedDate = value && typeof value.toDate === 'function' 
        ? formatter.format(value.toDate(getLocalTimeZone())) 
        : "Select date";

    return (
        <AriaDatePicker shouldCloseOnSelect={false} {...props} value={value} onChange={setValue} className={cx("w-full", typeof props.className === 'string' ? props.className : undefined)}>
            <AriaGroup className="w-full">
                <Button size="md" color="secondary" iconLeading={CalendarIcon} className="w-full">
                    {formattedDate}
                </Button>
            </AriaGroup>
            <AriaPopover
