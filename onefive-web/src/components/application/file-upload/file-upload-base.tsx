...
export interface FileListItemProps {
    ...
    onRetry?: () => void;
    /**
     * Additional actions to display alongside the delete button.
     * Can be used to add custom components like Select dropdowns.
     */
    actions?: React.ReactNode;
}

export const FileListItemProgressBar = ({ name, size, progress, failed, type, fileIconVariant, onDelete, onRetry, actions, className }: FileListItemProps) => {
        ...
        >
            ...
            <div className="flex min-w-0 flex-1 flex-col items-start">
                <div className="flex w-full max-w-full min-w-0 flex-1">
                    ...
                    <div className="flex items-center gap-2 -mt-2 -mr-2 self-start">
                        {actions && <div className="mt-2 mr-2">{actions}</div>}
                        <ButtonUtility color="tertiary" tooltip="Delete" icon={Trash01} size="xs" onClick={onDelete} />
                    </div>
                </div>
...
export const FileListItemProgressFill = ({ name, size, progress, failed, type, fileIconVariant, onDelete, onRetry, actions, className }: FileListItemProps) => {
    const isComplete = progress === 100;
