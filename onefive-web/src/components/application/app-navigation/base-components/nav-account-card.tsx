...
import { BookOpen01, Building07, ChevronSelectorVertical, LogOut01, Plus, Settings01, User01 } from "@untitledui/icons";
...
export const NavAccountMenu = ({
    className,
    accounts: _accounts = placeholderAccounts,
    ...
    const { data: userStartups, isLoading: startupsLoading } = useUserStartups();
    const [isCreateModalOpen, setIsCreateModalOpen] = useState(false);

    const displayStartups = userStartups || [];
    const isLoading = startupsLoading;
    ...
    return (
        <>
            ...
            >
                <div className="rounded-xl bg-primary ring-1 ring-secondary">
                    ...
                    <div className="flex flex-col gap-0.5 border-t border-secondary py-1.5">
                        <div className="px-3 pt-1.5 pb-1 text-xs font-semibold text-tertiary">Mes startups</div>

                        {isLoading ? (
                            <div className="flex items-center justify-center py-4 px-3">
                                <div className="text-sm text-tertiary">Chargement...</div>
                            </div>
                                    ...
                                    >
                                        <div className="flex items-center gap-2">
                                            ...
                                            <div className="flex-1 min-w-0">
                                                <div className="text-sm font-semibold text-secondary truncate">{startup.name}</div>
                                                {startup.tagline && (
                                                    <div className="text-xs text-tertiary truncate">{startup.tagline}</div>
                                                )}
                                            </div>
                                            <Badge
                                                variant={
                                                    startup.role === "SUPER_ADMIN"
                                                        ? "default"
                                                        : startup.role === "ADMIN"
                                                          ? "secondary"
                                                          : "outline"
                                            ...
                                            >
                                                {startup.role === "SUPER_ADMIN" ? "Admin" : startup.role === "ADMIN" ? "Admin" : "Membre"}
                                            </Badge>
                                        </div>
                                    </Link>
                                ))}
                            ...
                            <div className="flex flex-col gap-2 px-2 pt-0.5 pb-2">
                                <Button iconLeading={Plus} color="secondary" size="sm" onClick={() => setIsCreateModalOpen(true)}>
                                    Créer ma première startup
                                </Button>
                            </div>
                        )}
                    </div>
            ...
            <CreateStartupModal open={isCreateModalOpen} onOpenChange={setIsCreateModalOpen} />
        </>
    );
};
