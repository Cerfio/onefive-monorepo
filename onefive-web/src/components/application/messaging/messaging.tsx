...
export const MessageItem = ({ msg, showUserLabel = true, ...props }: MessageItemProps) => {
    ...
    return (
        <li key={msg.id} {...props} className={cx("relative flex items-start gap-3", msg.user?.me ? "justify-end pl-10" : "pr-8 lg:pr-10", props.className)}>
            {msg.user && !msg.user.me && <Avatar src={msg.user.avatarUrl} alt={msg.user.name!} size="md" status={msg.user.status} />}

            <article className={cx("flex min-w-0 flex-col gap-1.5", !msg.user?.me && "flex-1", msg.user?.me && "items-end")}>
                {((msg.user && showUserLabel) || msg.sentAt || msg.status) && (
                    <header className={cx("flex items-center gap-2", msg.user?.me && "flex-row-reverse")}>
                        {/* User label */}
