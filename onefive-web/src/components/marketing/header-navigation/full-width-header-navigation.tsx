...
export const Menu2ColWithSidebar = ({ className, isMobile }: { className?: string; isMobile?: boolean }) => {
    const megaMenuItems: { title: string; items: { title: string; subtitle?: string; shortSubtitle?: string; href: string; Icon: any; badge?: any }[] }[] = [
        {
            title: "Resources",
            items: [
                {
        ...
        <section className={cx("overflow-hidden bg-primary md:shadow-lg", className)}>
            <div className="mx-auto flex max-w-container flex-col md:px-8 md:pb-6 xl:flex-row xl:pb-0">
                <div className="flex flex-col gap-5 pt-4 pb-5 md:flex-row md:pt-6 md:pr-6 md:pb-6">
                    {megaMenuItems.map((nav) => (
                        <div key={nav.title} className="flex-1">
                            <h3 className="mb-3 px-4 text-sm font-semibold text-brand-tertiary md:px-0">{nav.title}</h3>
                            <ul className="flex flex-col gap-0.5">
                                {nav.items.map(({ title, subtitle, href, Icon, badge }) => (
                                    <li key={title}>
                    ...
                    <div className="flex flex-col items-start bg-secondary pt-5 pb-6 md:rounded-2xl md:p-6 md:pr-8">
                        ...
                        <div className="flex flex-col gap-2 md:gap-0.5">
                            <VideoCardHorizontal
                                ...
                                actionsContent={
                                    <Button href="/demo" color="link-color" size="sm" iconLeading={PlayCircle} className="inline-flex justify-start">
                                        Watch video
                                    </Button>
                                }
                            />
