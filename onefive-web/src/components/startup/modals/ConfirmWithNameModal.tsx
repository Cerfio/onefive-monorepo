...
interface ConfirmWithNameModalProps {
  open: boolean;
...
export const ConfirmWithNameModal: React.FC<ConfirmWithNameModalProps> = ({
  open,
  onOpenChange,
  title,
  description,
  ...
  const isMatch = inputValue.trim().toLowerCase() === nameToConfirm.trim().toLowerCase();
            ...
            <div className="relative w-full overflow-hidden rounded-2xl bg-primary shadow-xl">
              ...
              <div className="px-6 pt-6 pb-4">
                <div className="flex items-start gap-4">
                  <div className={`flex h-10 w-10 shrink-0 items-center justify-center rounded-full ${variantColors.icon}`}>
                ...
                <div className="mt-5">
                  <label className="block text-sm font-medium text-secondary mb-1.5">
                    Tapez <span className="font-bold text-primary">{nameToConfirm}</span> pour confirmer
                  </label>
                  <Input
                    value={inputValue}
                    onChange={(value) => setInputValue(value)}
                    placeholder={placeholder || nameToConfirm}
                    autoFocus
                  />
                </div>
              </div>
