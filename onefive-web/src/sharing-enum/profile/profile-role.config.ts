 ...
 * 
 * ⚠️ IMPORTANT: Cette configuration doit rester synchronisée avec:
 * - onefive-back/src/profile/profile-role.config.ts
 */
 ...
 */
export function getProfileRoleMetadata(
  role: ProfileRole,
): ProfileRoleMetadata | undefined {
...
/**
 * Récupère un tableau de tous les rôles avec leurs métadonnées
 * Utile pour les dropdowns, checkboxes, etc.
 */
export function getAllProfileRolesWithMetadata(): Array<{
  role: ProfileRole;
  metadata: ProfileRoleMetadata;
}> {
  return getAllProfileRoles().map((role) => ({
    role,
    metadata: PROFILE_ROLE_METADATA[role],
  }));
}

/**
 * Convertit une valeur d'enum ProfileRole en son label lisible (masculin par défaut)
 * @deprecated Utiliser getGenderedShortLabel avec le genderPreference
 */
export function getProfileRoleLabel(roleValue: string): string {
...
/**
 * Convertit un tableau de valeurs d'enum ProfileRole en leurs labels lisibles (masculin par défaut)
 * @deprecated Utiliser getGenderedShortLabel avec le genderPreference
 */
export function getProfileRoleLabels(roleValues: string[]): string[] {
  return roleValues.map(getProfileRoleLabel);
}
