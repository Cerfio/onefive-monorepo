# Untitled UI React — AGENT.md (référence)

Référence partagée pour **tous les projets frontend** du monorepo qui s’alignent sur le design system Untitled UI (travail offline, historique git). **Source canonique :** https://www.untitledui.com/react/AGENT.md — en cas d’écart, priorité à l’URL.

**Portée typique :** `onefive-web`, `onefive-backoffice`, `landing-page`, `onefive-bo-landing-page`. Pour `onefive-email/transactional` (React Email), appliquer surtout les principes généraux (clarté, accessibilité) ; le détail Tailwind / React Aria du guide vise les apps avec le kit Untitled UI.
