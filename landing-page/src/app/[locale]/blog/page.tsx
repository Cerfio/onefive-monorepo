...
const Body = () => {
  ...
  const itemsPerPage = 12; // Articles par page
    ...
    const loadBlogData = async () => {
      try {
        ...
        if (articlesResponse.docs && Array.isArray(articlesResponse.docs)) {
          ...
          setTotalPages(Math.ceil(articlesResponse.totalDocs / itemsPerPage));
        } else {
          console.error("Invalid response format:", articlesResponse);
  ...
  const renderPagination = () => {
    ...
    if (totalPages <= 7) {
      ...
      // Sinon on affiche une pagination avec ellipses
      pages.push(1);
        ...
        {pages.map((page, index) =>
          page === "..." ? (
            ...
            <Button
              ...
              className={`w-10 h-10 ${currentPage === page ? "bg-[#5E6AD2]" : ""}`}
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {isLoading ? (
          ...
          filteredArticles.map((article) => (
            <Link
              key={article.id || `article-${Math.random()}`} // Fallback unique key
            ...
            >
              <div className="relative h-56 overflow-hidden bg-gray-100">
                ...
                />
                <div className="absolute inset-0 bg-gradient-to-t from-black/40 to-transparent opacity-10 group-hover:opacity-100 transition-opacity duration-300" />
