...
export default function DiscussionsPage() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      const response = await api
        .get('admin/discussions', { searchParams: params })
        .json<{ data: DiscussionRow[]; total: number }>();
  ...
  const remove = async (discussionId: string) => {
    ...
    if (!ok) return;
    try {
      await api.delete(`admin/discussions/${discussionId}`);
  ...
  const toggleHide = async (discussionId: string, isHidden: boolean) => {
    try {
      await api.patch(`admin/discussions/${discussionId}/hide`, { json: { isHidden } });
      toast.success(isHidden ? 'Discussion masquée' : 'Discussion affichée');
      await load();
    } catch (err) {
  ...
  return (
    <div>
      <SectionHeader.Root>
        <SectionHeader.Group>
          <div>
            <SectionHeader.Heading>Modération discussions</SectionHeader.Heading>
            <SectionHeader.Subheading>
              Visualiser et modérer les discussions de la communauté.
            </SectionHeader.Subheading>
          </div>
      ...
      <form onSubmit={handleSearch} className="mb-4 flex flex-1 gap-2">
        <Input
      ...
      {!loading && !error && items.length === 0 && (
        <EmptyState>
          ...
          <EmptyState.Content>
            <EmptyState.Title>Aucune discussion</EmptyState.Title>
            <EmptyState.Description>
              {search ? `Aucun résultat pour "${search}"` : 'Aucune discussion à modérer.'}
            </EmptyState.Description>
          </EmptyState.Content>
        </EmptyState>
      )}
                    ...
                    <td className="whitespace-nowrap px-4 py-3">
                      <div className="flex gap-2">
                        <Badge color="gray" size="sm">
                          {item._count.answers} rép.
                        </Badge>
                        <Badge color="gray" size="sm">
                          {item._count.upvotes} upvotes
                        </Badge>
                      </div>
                    </td>
