...
describe('CreatePostDto', () => {
  describe('content validation', () => {
    ...
    it('should reject content exceeding maximum length', async () => {
      ...
      expect(tooLongContent.length).toBe(VALIDATION_LIMITS.POST.CONTENT_MAX + 1);
      const errors = await validate(dto);
  ...
  describe('medias validation', () => {
    ...
    it('should reject medias exceeding maximum count', async () => {
      ...
      expect(tooManyMedias.length).toBe(VALIDATION_LIMITS.POST.MEDIAS_MAX_COUNT + 1);
      const errors = await validate(dto);
      expect(errors.length).toBeGreaterThan(0);
  ...
  describe('PostMediaDto validation', () => {
    ...
    it('should reject media URL exceeding max length', async () => {
      const longUrl = 'https://example.com/' + 'a'.repeat(VALIDATION_LIMITS.POST.MEDIA_URL_MAX);
      const media = plainToClass(PostMediaDto, {
        url: longUrl,
        mimeType: 'image/jpeg',
        fileName: 'image.jpg',
      ...
      expect(longUrl.length).toBeGreaterThan(VALIDATION_LIMITS.POST.MEDIA_URL_MAX);
      const errors = await validate(media);
      expect(errors.length).toBeGreaterThan(0);
    ...
    it('should reject media fileName exceeding max length', async () => {
      const longFileName = 'a'.repeat(VALIDATION_LIMITS.POST.MEDIA_FILE_NAME_MAX + 1) + '.jpg';
      const media = plainToClass(PostMediaDto, {
        url: 'https://example.com/image.jpg',
        mimeType: 'image/jpeg',
      ...
      expect(longFileName.length).toBeGreaterThan(VALIDATION_LIMITS.POST.MEDIA_FILE_NAME_MAX);
      const errors = await validate(media);
      expect(errors.length).toBeGreaterThan(0);
    ...
    it('should reject media mimeType exceeding max length', async () => {
      const longMimeType = 'a'.repeat(VALIDATION_LIMITS.POST.MEDIA_MIME_TYPE_MAX + 1);
      const media = plainToClass(PostMediaDto, {
        url: 'https://example.com/image.jpg',
      ...
      expect(longMimeType.length).toBeGreaterThan(VALIDATION_LIMITS.POST.MEDIA_MIME_TYPE_MAX);
      const errors = await validate(media);
      expect(errors.length).toBeGreaterThan(0);
  ...
  describe('edge cases', () => {
    it('should handle exact boundary for all limits', async () => {
      ...
      const exactMedias = Array(VALIDATION_LIMITS.POST.MEDIAS_MAX_COUNT)
        ...
        .map((_, i) =>
          plainToClass(PostMediaDto, {
            url: 'https://example.com/' + 'a'.repeat(VALIDATION_LIMITS.POST.MEDIA_URL_MAX - 23),
            mimeType: 'a'.repeat(VALIDATION_LIMITS.POST.MEDIA_MIME_TYPE_MAX),
            fileName: 'a'.repeat(VALIDATION_LIMITS.POST.MEDIA_FILE_NAME_MAX - 4) + '.jpg',
            size: 1024,
          }),
        );
