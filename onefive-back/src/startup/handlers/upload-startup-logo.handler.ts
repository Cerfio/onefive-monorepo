...
import {
  StartupLogoMimeTypeException,
  StartupLogoFileSizeException,
} from '../startup.exception';
import { v4 as uuidv4 } from 'uuid';

@Injectable()
export class UploadStartupLogoHandler {
  constructor(
  ...
  async execute({
    ...
    if (!allowedMimeTypes.includes(file.mimetype)) {
      StartupLogoMimeTypeException.throw(this.logger, {
        transactionId,
        startupId,
    ...
    if (file.size > maxSize) {
      StartupLogoFileSizeException.throw(this.logger, {
        transactionId,
        startupId,
        fileSize: file.size,
        maxSize,
    ...
    const fileId = uuidv4();
    const fileKey = `avatars/startups/${startupId}/${fileId}`;
    ...
    // Mettre à jour la startup avec la nouvelle URL du logo
    ...
    this.logger.info('Startup logo uploaded successfully', {
      transactionId,
      startupId,
      userId,
